  ...
  async execute({ message, args, userData, getUserData }) {
    ...
    if (!subcommand || message.mentions.users.first()) {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          `> ${message.author} challenged ${opponent} to a battle.\n\n` +
          `> Both players may choose up to **4 characters** for their team.\n\n` +
          `__**Commands**__\n` +
    ...
    if (subcommand === 'team') {
      ...
      const embed = new EmbedBuilder()
        .setColor(0x000000)
        .setTitle(`${message.author.username.toUpperCase()}'S TEAM`)
    ...
    if (subcommand === 'attack') {
      ...
      const nextTurnEmbed = new EmbedBuilder()
        .setColor(0x000000)
        .setTitle(`ROUND ${battle.currentRound} — ${nextPlayer.username.toUpperCase()}'S TURN`)
        .setDescription(
          `> ${nextChar.name} — HP: ${nextChar.currentHealth}/${nextChar.maxHealth}\n\n` +
          `__**Choose your move**__\n${movesList}\n\n` +
          `-# Type \`.battle attack <move number>\` to attack.`
        );
