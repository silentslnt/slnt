...
module.exports = {
  ...
  async execute({ message, args, userData, saveUserData, updateUserBalance, logAdminAction, client }) {
    await ensureLoaded();
    ...
    if (sub === 'buy') {
      ...
      if (userData.balance < LOTTERY_PRICE)
        return message.channel.send(`You need at least ${LOTTERY_PRICE} coins to buy a lottery ticket.`);
      ...
      const boughtEmbed = new EmbedBuilder()
        .setTitle('LOTTERY TICKET BOUGHT')
        .setDescription(
          `> Spent **${LOTTERY_PRICE}** coins on a ticket.\n` +
          `> You now have **${currentTickets + 1}/5** tickets.`
        )
        .addFields(
          { name: 'Total Pot',    value: `**${lotteryState.pot.toLocaleString()}** coins`, inline: true },
    ...
    if (sub === 'draw') {
      ...
      const ticketCount = lotteryState.tickets.length;

      // Award winnings to winner
      if (winnerId === userId) {
        userData.balance += pot;
        await saveUserData({ balance: userData.balance });
      } else {
        await updateUserBalance(winnerId, pot);
      }

      let winnerUsername = winnerId;
      try {
        const fetched = client.users.cache.get(winnerId) ?? await client.users.fetch(winnerId);
        winnerUsername = fetched.username;
      } catch {}

      await logAdminAction(message.author.id, message.author.username, 'lottery', 'Lottery Draw', winnerId, winnerUsername, `pot ${pot.toLocaleString()} coins, ${ticketCount} tickets`);

      const winnerEmbed = new EmbedBuilder()
        .setTitle('LOTTERY DRAWN')
        .setDescription(
          `> <@${winnerId}> wins the raffle!\n` +
          `> They win the entire pot of **${pot.toLocaleString()}** coins.`
        )
        .setColor(BLACK)
        .setFooter({ text: message.guild?.name || 'Shiro' });
    ...
    return message.channel.send(
      `Usage: \`.lottery buy\` to buy a ticket (**${LOTTERY_PRICE}** coins, max 5 per user), \`.lottery status\` to check the pot, \`.lottery draw\` (admin only)`
    );
  }
};
